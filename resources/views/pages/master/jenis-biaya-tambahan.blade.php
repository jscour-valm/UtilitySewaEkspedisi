...
@section('content')
<div class="flex flex-col gap-4 pb-2" x-data="jenisBiayaTambahan()">
    <div class="rounded-xl bg-white shadow-sm p-6">
        <div class="flex items-center justify-between pb-4 border-b border-gray-200 mb-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Daftar Jenis Biaya</h1>
                <p class="text-gray-600 text-sm">Master referensi jenis biaya tambahan — dipakai KG pas nambahin biaya tambahan di pengajuan.</p>
            </div>
            <button
                type="button"
                ...
                <span x-show="!showForm">+ Tambah Jenis Biaya</span>
        ...
        <div x-show="showForm" class="bg-blue-50 rounded-lg p-4 border border-blue-200 mb-4">
            <h3 class="font-semibold text-gray-900 mb-4">
                <span x-show="!editingId">Tambah Jenis Biaya Baru</span>
                <span x-show="editingId">Edit Jenis Biaya</span>
            </h3>
            <div class="flex gap-3">
                <input
                    type="text"
                    x-model="formNamaBiaya"
                ...
                <button
                    type="button"
                    @click="saveForm()"
                    :disabled="!formNamaBiaya.trim() || isSaving"
                    :class="(!formNamaBiaya.trim() || isSaving) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-avian-green-dark'"
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($jenisBiaya as $jb)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 font-medium text-gray-800">{{ $jb->nama_biaya }}</td>
                        ...
                        <td class="px-4 py-3 text-right space-x-2">
                            <button
                                type="button"
                                @click="editItem({{ $jb->id_jenis_biaya }}, @js($jb->nama_biaya))"
                                class="text-xs font-medium text-avian-green hover:text-avian-green-dark transition">
                                Edit
                            </button>
                            <button
                                type="button"
                                @click="if(confirm('Hapus jenis biaya ini?')) deleteItem({{ $jb->id_jenis_biaya }})"
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="py-12 text-center text-sm text-gray-400">Belum ada jenis biaya terdaftar.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
function jenisBiayaTambahan() {
    return {
        ...
        editItem(id, nama) {
            this.editingId = id;
            this.formNamaBiaya = nama;
            this.showForm = true;
        },

        async saveForm() {
            if (!this.formNamaBiaya.trim()) return;

            this.isSaving = true;
            try {
                const method = this.editingId ? 'PUT' : 'POST';
                const url = this.editingId ? `/api/jenis-biaya/${this.editingId}` : '/api/jenis-biaya';
                ...
                const res = await fetch(url, {
                    ...
                    body: JSON.stringify({ nama_biaya: this.formNamaBiaya.trim() }),
                });
                ...
                console.error('Gagal simpan jenis biaya:', e);
                alert('Gagal menyimpan jenis biaya');
            } finally {
                this.isSaving = false;
            }
        },
        ...
        async deleteItem(id) {
            try {
                const res = await fetch(`/api/jenis-biaya/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content ?? '',
                ...
                console.error('Gagal hapus jenis biaya:', e);
